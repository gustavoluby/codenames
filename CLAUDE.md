...
- `lib/sound.ts` sintetiza tudo no Web Audio (sem arquivo de áudio): voto, dica, entrar num time (carimbo no dossiê — versão cheia para você, discreta para os outros), troca de vez, acerto, lead frio, carta do adversário, Churn, palmas na vitória, trombone de derrota ("quan quan quan quaaan") e o baralho sendo embaralhado e distribuído (atrito de papel + tapinha na mesa + panorâmica pelos lados).
